...
So why bother with CSS if our websites can already be viewed as plain HTML pages. Well, CSS allows us to make content more visually appealing, it gives a website an identity, differentiating it from others making it memorable. It makes content easily digestible when consideration is given to the treatment of typography.

It would take a separate website to run through everything we need to cover for CSS but luckly others have done the job for us. The links **Resources** will take you through a number of useful books and websites that should prove invaluable.

In additional to a general introduction to CSS we looked at **Common Typographic Errors** and how we should fix them. Today’s web provides the designer with many more possibilities from early days of restricted, standard system font sets. Nowadays we have access to a growing range of web fonts and we can include ligatures, add font variants, kern our type, add swashes and much more. 

Our very own Christopher Murphy and Typekit designer, Jake Giltsoff, took ther Baskerville project to a wider audience via a [**net magazine article**](http://www.creativebloq.com/web-design/create-beautiful-web-typography-111517663) looking at how many of the finer aspects of the text could and should be presented. 

Taking this a little step further we looked at how these finer details can be presented with a little CSS. So, in no particular order:

####Puncutation

- **Baskerville's work** – notice anythong wrong with this text? No? The sharper eyed will notice that the apostrophe is wrong. What we see here, and commonly see in many cases is a [Prime](https://printingpartners.wordpress.com/2011/06/03/the-apostrophe-prime-acute-double-prime-and-quotation-mark) and is the wrong character. On many occasions it’s enough to use the standard keyboard symbol. On a Apple kepboard this can be found by pressing the **alt + shift + right curly bracket keys** – ’ or you could use `&rsquo;` or `&#8217;` – an [HTML character code](http://www.ascii.cl/htmlcodes.htm), to make sure it works across all browsers (if you have to work with a lot of text this last approach can become very time consuming) generally we are pretty safe to use the standard keyboard form.
