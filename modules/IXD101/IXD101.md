...
The [**Nine Squares**](resources/nine_squares.pdf) exercise was introduced this week where we would like you to create two separate pieces using only dots on one and only lines on the other. You are not restructed on the number of dots or lines you can use but. Try to look at the 3x3 grid as a whole and treat each square as a component of the final image.
...
First, we took brief look at the history of the world wide web, from the [first web page](http://info.cern.ch) to inventor [Sir Tim Berners-Lee](http://webfoundation.org/about/vision/history-of-the-web) original proposal for the web to what it has become today.

We took a look at the development of the [web browser](http://www.telegraph.co.uk/technology/microsoft/11577364/Web-browsers-a-brief-history.html) and the chaos that ensued, to the establishment of the [Web Standards Project](http://www.webstandards.org) and the move towards a open and accessible web.
...
We started off with a look at the foundations of a typical web page starting with the head content. The `<head>` element contains information (metadata) about the document such as the title, scripts, and style sheets. Elements which can be used inside the `<head>` are:
- `<title>`
- `<base>`
- `<link>`
- `<style>`
...
While some of this content is standard across all websites, specific information  such as links to web fonts, metadata and scripts will be unique to each project.

As a starting point, the version presented on my GitHub [Pattern Library](https://github.com/eleventhirty/pattern_library/blob/gh-pages/objects/head.html) will set you on the right path.



### John Baskerville

To introduce you to the basics of HTML and CSS we kicked off the John Baskerville project which will run over the next six week. If you need a little help with this first introduction to HTML you can takes a look at [my version](https://github.com/eleventhirty/john_baskerville) for reference. Please don’t copy and paste this, you will not learn anything, use the file for reference to create your own version.
...
We touched on naming conventions, to make sure you follow standard procedures for naming files for online content this article – [How to Name HTML Files](https://www.lifewire.com/naming-html-files-3466503) – will tell you all you need to know

### Pattern Libraries

The value and importance of pattern libraries was touched on today. The following from Paul Boag helps explain what they are – [How to create a pattern library and why you should bother](https://boagworld.com/design/pattern-library/). The Medium post – [Pattern Libraries](https://medium.com/@whatjackhasmade/pattern-libraries-abcc45c6144c) – provides a good overview of what they are. 

For a broad overview of pattern libraries and style guides [Website Style Guide Resources](http://styleguides.io/) offer a wealth of examples, articles, books and tools.

**Make sure you read both articles to ensure you have a grasp of their value in the industry.** 

### Homework – History of Type 

To help embed what we covered today I would like you to take the [brief-history-of-type.txt](https://github.com/ixdbelfast/ixdbelfast.github.io/blob/master/modules/IXD101/resources/brief-history-of-type.txt) from GitHub and mark it up to match the [image provided](https://github.com/ixdbelfast/ixdbelfast.github.io/blob/master/modules/IXD101/resources/html-01-make-this.jpg). Add this to your Baskerville Repo under a separate heading **History of Type**.

You can take a look at [my repo](https://github.com/eleventhirty/john_baskerville) as a reference but **DO NOT COPY** you will learn nothing from simply copying and pasting other peoples material. The best way to learn is to do it yourself.

### Pattern Libraries

In addition to your weekly tasks we want you to start building your own pattern library which you will use you demonstrate your knowledge of HTML and CSS over the coming weeks. 

To start you off you need to create a new repository on your GitHub accould naming it – **pattern_library** – make sure you clone it on your desktop version of GitHub.

On your desktop version add folders:
- objects
- components
- patterns
- pages

Over the following weeks we will be adding content to these folders.

To kick things off you need to create two files – head.html and typography.html both of which should be saved in the **objects** folder. To make it easy for you, you can reference [my version](https://github.com/eleventhirty/pattern_library) this week 
...
**Web**
+ [HTML Basics](https://developer.mozilla.org/en-US/Learn/Getting_started_with_the_web/HTML_basics) – an excellent introduction to HTML
...
+ [The Difference Between Block-level and Inline Elements](http://www.impressivewebs.com/difference-block-inline-css/) click on the HTML Elements link under **References** to see the list of available elements.
...
#### Homework
This week we would like you to put today's class into practice by taking the plain text [Type History List](resources/homework2/history-list.txt) and recreating it to look like [this image](resources/homework2/history_list.jpg). When complete add the marked up list to the end of your Brief History of Type page before the footer and save it as Version 2.
...
### Homework
For this week you should save a new version of the History of Type page and convert last weeks list to a table like the [example here](resources/make-this-index.png). Also in the resources folder you will see an [unedited-images](resources/unedited-images) folder. I have placed a number of royalty-free images for each of the type designers in here. You will need to re-size these to a common size and save them in appropriate, optomised formats and place them below the title in each section.

If you didn't do so during class, please also add a relative link at the end of the John Baskervile section to today's John Baskerville page.

**IMPORTANT** Please make sure you keep checking each version of your Baskerville and Type History pages through an [HTML validator](https://validator.w3.org/). This will ensure your markup is correct, I will be testing pages at random during the assessment period to make sure they are 100% correct.
...
By now we should have a solid understanding of the fundamentals of markup, so now it’s time to start to apply some design aesthetics to our text and images. 
...
We can see the same issue here – **"Having been an early admirer of the…proportion."** where the quotation has used a double prime instead of proper double quotation marks. These can be found by typing **alt + shift + left curly bracket** for – **”** and **alt + left curly bracket** for – **“**. Once again there are alternative HTML codes for both – **&ldquo;** and **rdqou;** for those instances where the standard symbol doesn’t work.
...
Week 10 – Web Typography
------------------------

This week we took a broad look at web typography with the introduction of [Google Fonts](https://fonts.google.com) and [Adobe Fonts](https://fonts.adobe.com).

Be sure to watch Tim Brown’s video on web typography which can be found on his [Universal Typography](http://universaltypography.com/) site, it is packed with excellent content.


### Resources
#### Books
- [Better Web Typography for a Better Web](https://betterwebtype.com/web-typography-book)
- [Type on Screen](https://www.amazon.co.uk/Type-Screen-Critical-Designers-Developers/dp/161689170X)
- [On Web Typography](https://abookapart.com/products/on-web-typography)
- [Web Typography](http://book.webtypography.net/)

#### Websites
- [Better Web Type](https://betterwebtype.com) a supporting website for the book plus a free course
...
- [Syntax for OpenType features in CSS](https://helpx.adobe.com/fonts/using/open-type-syntax.html#calt)

---

Week 11 – Floats and Sprinkles
-------------------------------------
...
- While we didn't cover it, [floats](https://css-tricks.com/all-about-floats) are a useful feature to check out. 

### A little extra…
If you want to add a simple gallery to your Baskerville website showing off a range of type foundry versions of the iconic typeface. The [ResponsiveSlides.js](http://responsiveslides.com) will be the easiest way to do this but feel free to try others if you’re feeling more adventurous.
...
Flexbox is one of the more recent features which is enjoying widespread support across most browsers. Chris Coyer has an excellent introduction to flexbox in [A Complete Guide to Flexbox](https://css-tricks.com/snippets/css/a-guide-to-flexbox)

#### Flexbox resources
- [Using CSS Flexible Boxes](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Flexible_Box_Layout/Using_CSS_flexible_boxes)
- [An Introduction to Flexbox](https://slicejack.com/introduction-to-flexbox)
- [Flexbox Froggy](http://flexboxfroggy.com/) – students have found this a great introduction to flexbox.
...
- [Understanding Flexbox](https://ohansemmanuel.github.io/uf_download.html) – the PDF file from the article above.
...
**My [GitHub](https://github.com/eleventhirty/john-baskerville)** has a number of the exercises we ran through over the last seven weeks
